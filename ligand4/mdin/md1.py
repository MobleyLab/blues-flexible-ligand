...
#ala2_solv = load_file('2gmx_wat.prmtop', xyz='2gmx_wat.inpcrd')
inpcrdName='rst/step8.rst.250000';
ala2_solv = load_file('complex_wat.prmtop', xyz=inpcrdName );
...
if restraints :
    ...
    if restraintType == "heavy atom" :
       for idx, atom_crd in enumerate(ala2_solv.positions):
           if idx < restraintAtom :
               name=ala2_solv.atoms[idx].name;
               if name[0] in ('C', 'N', 'O'):
                  xyz = ala2_solv.positions[idx].in_units_of(u.nanometers)/u.nanometers
...
#Restart reporter
restrt = RestartReporter('rst/'+outFname+'.rst', rstInterval, ala2_solv.ptr('natom') );
...
if restartSim:
    ...
    velocities = ala2_solv.velocities;

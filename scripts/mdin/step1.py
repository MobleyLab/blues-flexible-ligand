...
restraintAtom = 4886;                                                       
restraintWt = 500;
restraintType = "heavy atom"
#filename
outFname='step1';
mdoutInterval = 100 #for mdout file
netcdfInterval = 5000 #for mdout file
rstInterval = 5000 #for rst file
...
if restraints :
    ...
    if restraintType == "heavy atom" :
       for idx, atom_crd in enumerate(complex_solv.positions):
           if idx < restraintAtom :
               name=complex_solv.atoms[idx].name;
               if name[0] in ('C', 'N', 'O'):
                  xyz = complex_solv.positions[idx].in_units_of(u.nanometers)/u.nanometers
                  force_restr.addParticle(idx, xyz)
                  #print(idx);
                  #print(mdData.atoms[idx]);
...
sim = app.Simulation(complex_solv.topology, system, integrator, platform)
...
if restartSim:
    print('RESTARTING simulation from a previous State..........')
